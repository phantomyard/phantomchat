# CLAUDE-RULES.md — PhantomChat.chat Subsystem Rules
...
- `getSelf()` returns `undefined` in PhantomChat mode (no MTProto auth) — guard all `.id` access.
...
`phantomchatIntercept()` in `apiManager.ts` routes Worker method calls: **Static** (`PHANTOMCHAT_STATIC` — `help.getConfig`, `updates.getState`, `account.*`, `stories.*`) return shaped stubs; **Bridge** (`PHANTOMCHAT_BRIDGE_METHODS` — getHistory, getDialogs, search, deleteMessages, sendMessage, sendMedia, getContacts, getUsers, getFullUser, editMessage) forward via `port.invoke('phantomchatBridge', ...)` to main-thread `apiManagerProxy` → `PhantomChatMTProtoServer.handleMethod()` → `message-store.ts` (IndexedDB) → native MTProto response. Worker processes normally via `saveMessages()` → `setMessageToStorage()` → mirror → UI. `PhantomChatSync` handles incoming ChatAPI messages → persist → dispatch `phantomchat_new_message`. Design principle: vanilla tweb code works unchanged; the bridge is transparent. Debug: `window.__phantomchatMTProtoServer`.
...
| `PHANTOMCHAT_ACTION_PREFIXES` in `apiManager.ts` must NOT contain `.get` or `.check` | Query methods need shaped responses, not `return true` |
...
| `window.__phantomchatOwnPubkey` must be set in `phantomchat-onboarding-integration.ts` | `contacts.ts` needs it to persist conversations |
| `saveApiUser()` preserves P2P synthetic user's `first_name` | Prevents bridge responses overwriting nicknames with hex fallbacks |
| `phantomchat_new_message` handler must build messages via `mapper.createTwebMessage()` directly | Never re-read from message-store — IndexedDB round-trip has 0-5s latency and silently drops messages |
| Call `rs.managers.appMessagesManager.invalidateHistoryCache(peerId)` after `phantomchat_new_message` arrives | Resets `SlicedArray`; without it, reopened chats return stale history |
...
| `PhantomChatSync.onIncomingMessage()` MUST save with `eventId = msg.relayEventId \|\| msg.id` | Mismatched eventIds (parsed `chat-XXX-N` vs rumor hex) → duplicate rows → two bubbles |
...
| VMT `sendMessage` must return `phantomchatMid` + `phantomchatEventId` | Worker's P2P shortcut renames temp mid `0.0001` → real timestamp mid; without this, outgoing bubbles sort wrong |
...
| `messages.editMessage` MUST be in `PHANTOMCHAT_BRIDGE_METHODS` | Otherwise `.edit` action prefix short-circuits it |

**P2P edit protocol**: edits are new NIP-17 gift-wraps carrying `['phantomchat-edit', '<originalAppMessageId>']` — the `chat-XXX-N` form, NOT rumor hex. Sender rows use it as `eventId`, receiver rows as `appMessageId`, so a single `getByAppMessageId` lookup works on both sides. Receive handler upserts the original row preserving `mid`/`twebPeerId`/`timestamp`; only `content` + `editedAt` change. Author verification mandatory: drop edits where `rumor.pubkey !== original.senderPubkey`.
...
- **Receive chain**: relay WS → `NostrRelay.handleEvent()` → gift-wrap decrypt → `RelayPool.handleIncomingMessage()` → `ChatAPI.handleRelayMessage()` → `PhantomChatSync.onIncomingMessage()` → `message-store` → `phantomchat_new_message` → `history_append` → bubble.
...
- `phantomchat_delivery_update` handled by `phantomchat-delivery-ui.ts`, maps `eventId → mid` via `PhantomChatPeerMapper.mapEventId(eventId, timestamp)`.

## Logout & Data Cleanup

- Settings logout calls `showLogOutPopup()` from `@components/popups/logOut` — never inline `indexedDB.deleteDatabase + reload`. `phantomchat-cleanup.ts` is the centralized module.
...
- PhantomChat IndexedDB: `phantomchat-messages`, `phantomchat-message-requests`, `phantomchat-virtual-peers`, `phantomchat-groups`, `PhantomChatPool`, `PhantomChat.chat`.
- PhantomChat localStorage: `phantomchat_identity`, `phantomchat-relay-config`, `phantomchat-last-seen-timestamp`, `phantomchat:read-receipts-enabled`.
...
- **Reset Local Data** (sibling of logout): `showResetLocalDataPopup()` in `src/components/popups/resetLocalData.ts` wipes everything except the seed via `clearAllExceptSeed()` in `phantomchat-cleanup.ts` and calls `apiManager.logOut(undefined, {keepPhantomChatIdentity: true})` so the Worker-side `deleteEncryptedIdentity()` is skipped. A `sessionStorage` marker (`phantomchat-just-reset`) triggers a confirmation toast on the next boot via `maybeShowResetToast()` called from `src/index.ts`.

## UI Components

- The active "Add Contact" dialog is in `src/components/sidebarLeft/tabs/contacts.ts` (imperative DOM), NOT `src/components/phantomchat/AddContact.tsx` (Solid.js — unused).
- `bubbles.ts` is 11000+ lines. `appMessagesManager.ts` is 8500+ lines. Changes to these files risk cascading side effects.
- All `notDirect` flags were removed from `contextMenu.ts` — all chats are PhantomChat, there are no Telegram DMs. The type field, invocation logic, and all 10 button properties were deleted.
- Hamburger profile entry (`buildPhantomChatProfileMenuContent` in `sidebarLeft/index.ts`): the async storage-read path must generate a dicebear avatar from the stored npub *before* calling `fetchOwnKind0`, otherwise fresh-onboarding (no cache, no kind 0 picture) leaves `avatar.src=""` until the user opens the profile tab.

## PhantomChat Module Architecture

`phantomchat-onboarding-integration.ts` is a thin orchestrator (~240 lines) wiring: `phantomchat-message-handler.ts` (incoming message builder), `phantomchat-pending-flush.ts` (queue for closed-chat peers), `phantomchat-read-receipts.ts` (batch on peer open), `phantomchat-delivery-ui.ts` (bubble sent/delivered/read icons). `chat-api-receive.ts` extracts `handleRelayMessage` with `ReceiveContext` DI as pure step functions (`isDeleteNotification`, `parseMessageContent`, `extractFileMetadata`, `isDuplicate`). All PhantomChat rootScope events are typed in `BroadcastEvents` (rootScope.ts) — no `as any` casts.

## Background Push Notifications

- Client subscribes via `phantomchat-push-client.ts` to a self-hosted Nostr → Web Push relay (NOT in this repo). Default endpoint `https://push.phantomchat.chat`; configurable via Settings → Notifications → Advanced.
- Server source: https://github.com/phantomchat-chat/nostr-webpush-relay (AGPL-3.0, Node.js). HTTP contract in that repo's `docs/PROTOCOL.md`.
- Auth: NIP-98 signed events (`buildNip98Header` in `phantomchat-push-client.ts`).
- SW handler: `src/lib/serviceWorker/phantomchat-push.ts` — discriminates on `payload.app === 'phantomchat-webpush-relay'`. Decryption gated by user's preview level (A=generic / B=full / C=sender-only); when A, privkey is never read in SW.
- SW-safe identity loader: `phantomchat-identity-sw.ts` (IDB-only, no `localStorage` fallback — SW context lacks it).
- Privacy disclosure: [`PUSH-NOTIFICATIONS.md`](PUSH-NOTIFICATIONS.md).

## MTProto Intercept (`apiManager.ts`)

- `phantomchatIntercept()` tries dynamic server first (main thread only), then checks `PHANTOMCHAT_STATIC`, then `PHANTOMCHAT_BRIDGE_METHODS` (Worker→Main via MessagePort), then action prefixes, then fallback `{pFlags: {}}`.
- `PHANTOMCHAT_STATIC` must return properly shaped responses — `{pFlags: {}}` causes "Cannot read properties" errors in managers.
...
- New tests in `src/tests/fuzz/` are auto-discovered; new tests in `src/tests/phantomchat/` must be appended to the explicit file list in `package.json` `test:phantomchat:quick`.
...
- In a persistent context the installed Service Worker intercepts `/src/...` requests and 404s them — the dynamic-import pattern from `e2e-reactions-bilateral.ts` (`import('/src/lib/phantomchat/...')`) won't work. Use UI-driven Playwright clicks/fills instead.
