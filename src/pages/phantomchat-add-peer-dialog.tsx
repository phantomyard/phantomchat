/**
 * PhantomChat.chat Add Peer Dialog
 *
 * Minimal modal dialog that lets users initiate a P2P chat by pasting an OwnID.
 * Follows the PhantomChatOnboarding vanilla-DOM pattern.
 */

import {MOUNT_CLASS_TO} from '@config/debug';
import './phantomchat-onboarding-tweb.css';

const LOG_PREFIX = '[PhantomChatAddPeerDialog]';
...
export function showAddPeerDialog(): void {
  ...
  const dialog = PhantomChatAddPeerDialog.create((ownId) => {
  ...
  (window as any).__phantomchatAddPeerDialog = dialog;
  dialog.show();
}
 ...
 */
export class PhantomChatAddPeerDialog {
   ...
   */
  static create(onConnect: (ownId: string) => void): PhantomChatAddPeerDialog {
    // Create overlay
    const overlay = document.createElement('div');
    overlay.className = 'phantomchat-dialog-overlay';
    ...
    container.className = 'phantomchat-dialog';

    overlay.appendChild(container);
    document.body.appendChild(overlay);

    return new PhantomChatAddPeerDialog(container, overlay, onConnect);
  }
  ...
  show(): void {
    ...
    requestAnimationFrame(() => {
      const input = this.container.querySelector<HTMLInputElement>('#phantomchat-ownid-input');
      input?.focus();
    });
  }
  ...
  hide(): void {
    ...
    const input = this.container.querySelector<HTMLInputElement>('#phantomchat-ownid-input');
    const errorEl = this.container.querySelector('#phantomchat-ownid-error') as HTMLElement | null;
    if(input) {
      input.value = '';
  ...
  private render(): void {
    ...
    this.container.innerHTML = `
      <div class="phantomchat-dialog-header">
        <h2 style="
          margin: 0 0 0.5rem 0;
          font-size: 1.25rem;
          font-weight: 700;
      ...
      <div class="phantomchat-dialog-body">
        <label for="phantomchat-ownid-input" style="
          display: block;
          font-size: 0.75rem;
        ...
        <input
          ...
          "
        />
        <p id="phantomchat-ownid-error" style="
          color: #ff6b6b;
          font-size: 0.75rem;
          margin: 0.5rem 0 0 0;
          display: none;
      ...
      <div class="phantomchat-dialog-actions" style="
        display: flex;
        gap: 0.75rem;
        margin-top: 1.5rem;
      ">
        <button
          id="phantomchat-dialog-cancel"
          style="
            flex: 1;
            padding: 0.875rem 1rem;
            border: 2px solid #2d3748;
  ...
  private attachEventListeners(): void {
    const cancelBtn = this.container.querySelector('#phantomchat-dialog-cancel') as HTMLButtonElement;
    const connectBtn = this.container.querySelector('#phantomchat-dialog-connect') as HTMLButtonElement;
    const input = this.container.querySelector('#phantomchat-ownid-input') as HTMLInputElement;
    const errorEl = this.container.querySelector('#phantomchat-ownid-error') as HTMLElement;
