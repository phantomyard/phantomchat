...
rootScope.addEventListener('phantomchat_identity_loaded', (data) => {
  setNpub(data.npub);
...
rootScope.addEventListener('phantomchat_identity_locked', () => {
  setIsLocked(true);
});

rootScope.addEventListener('phantomchat_identity_unlocked', (data) => {
  setNpub(data.npub);
  setIsLocked(false);
});

rootScope.addEventListener('phantomchat_identity_updated', (data) => {
...
export default function usePhantomChatIdentity() {
  return {
    npub,
    displayName,
    nip05,
