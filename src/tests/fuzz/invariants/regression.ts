...
export const noNip04: Invariant = {
  ...
  async check(ctx: FuzzContext): Promise<InvariantResult> {
    ...
    if(nip04.length > 0) {
      return {ok: false, message: `found ${nip04.length} kind 4 (NIP-04) events on relay — PhantomChat must use NIP-44 (kind 1059 gift-wrap)`, evidence: {kindCounts: {nip04: nip04.length, total: events.length}}};
    }
    return {ok: true};
  }
};
...
const DUMP_IDENTITY_IDB = async() => {
  try {
    ...
    if(!db.objectStoreNames.contains('phantomchat_identity')) {
      db.close();
      return '';
    }
    const tx = db.transaction('phantomchat_identity', 'readonly');
    const store = tx.objectStore('phantomchat_identity');
  ...
  async check(ctx: FuzzContext): Promise<InvariantResult> {
    for(const id of ['userA', 'userB'] as const) {
      ...
      if(/\bnsec1[0-9a-z]{20,}/.test(dump)) {
        return {ok: false, message: `plaintext nsec1… found in phantomchat_identity IDB on ${id}`, evidence: {user: id, dumpSample: dump.slice(0, 200)}};
      }
      ...
      if(/\b[a-z]{3,12}\b(?:\s+\b[a-z]{3,12}\b){3,}/.test(dump)) {
        ...
        if(!hasCrypto) {
          return {ok: false, message: `plaintext seed phrase pattern found in phantomchat_identity IDB on ${id} (no ciphertext markers)`, evidence: {user: id, dumpSample: dump.slice(0, 200)}};
        }
      }
    }
    return {ok: true};
