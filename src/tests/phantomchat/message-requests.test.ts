...
describe('Message Requests', () => {
  ...
  describe('acceptRequest', () => {
    test('calls messageRequestStore.acceptRequest with pubkey', async() => {
      const store = (await import('@lib/phantomchat/message-requests')).getMessageRequestStore();
  ...
  describe('phantomchat_message_request event', () => {
    test('incoming event triggers listener for new request', async() => {
      ...
      rootScope.dispatchEvent('phantomchat_message_request', {
    ...
    test('multiple events add multiple entries', async() => {
      ...
      const received: any[] = [];
      rootScope.addEventListener('phantomchat_message_request', (data: any) => {
        received.push(data);
      });
