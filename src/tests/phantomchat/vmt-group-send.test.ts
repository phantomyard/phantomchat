 ...
 * returns an `updates`-shaped response carrying `phantomchatMid` +
 * `phantomchatEventId` so the Worker's post-send shortcut renames the temp
...
vi.mock('@lib/phantomchat/group-api', () => ({
  getGroupAPI: () => ({
...
describe('VirtualMTProtoServer.sendMessage — group branch', () => {
  ...
  beforeEach(async() => {
    ...
    vi.doMock('@lib/phantomchat/group-api', () => ({
      getGroupAPI: () => ({sendMessage: mockGroupSendMessage})
    }));
    vi.doMock('@lib/phantomchat/phantomchat-peer-mapper', () => ({
      PhantomChatPeerMapper: class {
        mapEventId = mockMapEventId;
        mapPubkey = vi.fn();
        createTwebMessage = vi.fn();
  ...
  it('delegates to GroupAPI.sendMessage and returns phantomchatMid/phantomchatEventId', async() => {
    ...
    expect(result.phantomchatMid).toBe(MID);
    expect(result.phantomchatEventId).toBe(RUMOR_ID);
  });
  ...
  it('returns emptyUpdates when no group record matches the peerId', async() => {
    ...
    expect(result._).toBe('updates');
    expect(result.phantomchatMid).toBeUndefined();
    expect(result.phantomchatEventId).toBeUndefined();
