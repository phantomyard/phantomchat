 ...
 * IndexedDB row (`phantomchat-messages`) to carry the same `mid`. Before the fix,
 * the sender-side send pipeline wrote the `phantomchat-messages` row twice:
 *
 *   1. `ChatAPI.sendMessage()` — saved a PARTIAL row ({eventId, content, …},
 *      no mid, no twebPeerId, no isOutgoing).
 *   2. `PhantomChatMTProtoServer.sendMessage()` — after sendText() returned, saved
...
// Polyfill Number.toPeerId used by PhantomChatPeerMapper.createTwebMessage so the
  ...
  it('sender pipeline: VMT.sendMessage → IDB row has mid + twebPeerId + isOutgoing:true', async() => {
    ...
    const mockChatAPI = {
      ...
      sendText: vi.fn().mockImplementation(async(content: string, opts?: {twebPeerId?: number; timestampSec?: number}) => {
        ...
        const mid = await PhantomChatBridge.getInstance().mapEventIdToMid(eventId, timestampSec);
        const row: any = {
          eventId,
