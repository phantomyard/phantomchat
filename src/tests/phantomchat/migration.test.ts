...
beforeAll(async() => {
  ...
  const migrationMod = await import('@lib/phantomchat/migration');
  needsMigration = migrationMod.needsMigration;
  migrateOwnIdToNpub = migrationMod.migrateOwnIdToNpub;

  const keyStorageMod = await import('@lib/phantomchat/key-storage');
  loadEncryptedIdentity = keyStorageMod.loadEncryptedIdentity;

  const queueMod = await import('@lib/phantomchat/offline-queue');
  loadAllQueuedMessages = queueMod.loadAllQueuedMessages;

  const identityMod = await import('@lib/phantomchat/nostr-identity');
  importFromMnemonic = identityMod.importFromMnemonic;

  const cryptoMod = await import('@lib/phantomchat/nostr-crypto');
...
/**
 * Open PhantomChat.chat DB at version 2 (matches key-storage) with all stores.
 * This avoids version conflicts between test seeding and production code.
 */
function openPhantomChatDB(): Promise<IDBDatabase> {
  return new Promise((resolve, reject) => {
    const request = indexedDB.open('PhantomChat.chat', 2);
...
async function seedOldIdentity(record: any): Promise<void> {
  const db = await openPhantomChatDB();
...
function openQueueDB(): Promise<IDBDatabase> {
  return new Promise((resolve, reject) => {
    const request = indexedDB.open('phantomchat-offline-queue', 1);
    request.onerror = () => reject(request.error);
    request.onsuccess = () => resolve(request.result);
