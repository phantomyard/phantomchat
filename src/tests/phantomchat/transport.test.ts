/**
 * Tests for PhantomChat.chat transport layer - WebRTC peer connection manager
 */
...
import {
  ...
  createPhantomChatPeerConnection,
  createPeerChannel,
  wrapPeerChannel
} from '@lib/phantomchat/peer';
import {
  PerfectNegotiation,
  createPoliteNegotiation,
  createImpoliteNegotiation
} from '@lib/phantomchat/peerNegotiation';
import {
  PeerTransport,
  createPeerTransport,
  TransportMessage,
  TransportState
} from '@lib/phantomchat/transport';
...
describe('createPhantomChatPeerConnection', () => {
  test('produces a connection with correct ICE config', () => {
    const {connection} = createPhantomChatPeerConnection();
  ...
  test('ICE config includes STUN servers', () => {
    // Verify PhantomChatIceConfig has STUN servers
    expect(PhantomChatIceConfig.iceServers).toBeDefined();
    expect(PhantomChatIceConfig.iceServers.length).toBeGreaterThan(0);

    // Check for Google STUN servers
    const hasGoogleStun = PhantomChatIceConfig.iceServers.some(server =>
  ...
  test('allows custom ICE config to override defaults', () => {
    ...
    const {connection} = createPhantomChatPeerConnection(customConfig);
    expect(connection).toBeDefined();
