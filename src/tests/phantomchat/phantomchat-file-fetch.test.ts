...
describe('phantomchat-file-fetch', () => {
  ...
  beforeEach(() => { origFetch = globalThis.fetch; clearPhantomChatFileCache(); });
  ...
  it('fetches, decrypts, and caches', async() => {
    ...
    const blob1 = await fetchAndDecryptPhantomChatFile('https://x/a', keyHex, ivHex);
  ...
  it('rejects on bad key', async() => {
    ...
    await expect(
      fetchAndDecryptPhantomChatFile('https://x/c', '11'.repeat(32), ivHex)
    ).rejects.toThrow();
  });
});
