 ...
 *
 * `ensureGroupChatInjected` (phantomchat-groups-sync.ts) writes the group Chat
...
describe('cleanupGroupChatInjection', () => {
  ...
  it('removes the group peer + chat from mirrors', async() => {
    const {cleanupGroupChatInjection} = await import('@lib/phantomchat/phantomchat-groups-sync');
