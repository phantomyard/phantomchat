...
beforeAll(async() => {
  ...
  const ptMod = await import('@lib/phantomchat/privacy-transport');
...
describe('PrivacyTransport — mode-based lifecycle', () => {
  ...
  it('dispatches phantomchat_tor_state with RuntimeState values', async() => {
    localStorage.setItem('phantomchat-tor-mode', 'off');
    const webtor = createMockWebtorClient();
    transport = new PrivacyTransport(pool as any, queue as any, webtor as any);
    await transport.bootstrap();
    ...
    const torCalls = calls.filter(([name]: [string, ...unknown[]]) => name === 'phantomchat_tor_state');
