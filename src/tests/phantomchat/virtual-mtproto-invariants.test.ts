...
import {
  ...
  P2P_PEER_ID_MIN
} from '@lib/phantomchat/bridge-invariants';

// ─── Rule 2: PHANTOMCHAT_ACTION_PREFIXES must not contain `.get`/`.check` ──
...
// ─── Rule 15: PHANTOMCHAT_BRIDGE_METHODS must contain required entries ─────
...
describe('Rule 12: sendMessage response shape', () => {
  it('accepts a response with both phantomchatMid and phantomchatEventId', () => {
    const result = validateSendMessageResponse({
      phantomchatMid: 1700000042,
      phantomchatEventId: 'abcdef1234567890'
    });
    expect(result.ok).toBe(true);
  });

  it('rejects a response missing phantomchatMid', () => {
    const result = validateSendMessageResponse({phantomchatEventId: 'abc'});
    expect(result.ok).toBe(false);
    if(!result.ok) expect(result.reason).toContain('phantomchatMid');
  });

  it('rejects a response with empty phantomchatEventId', () => {
    const result = validateSendMessageResponse({phantomchatMid: 1, phantomchatEventId: ''});
    expect(result.ok).toBe(false);
    if(!result.ok) expect(result.reason).toContain('phantomchatEventId');
  });

  it('rejects a response with wrong type phantomchatMid', () => {
    const result = validateSendMessageResponse({phantomchatMid: '1', phantomchatEventId: 'abc'});
    expect(result.ok).toBe(false);
  });
});
...
describe('Production config currently satisfies the rules', () => {
  ...
  it('PHANTOMCHAT_ACTION_PREFIXES in apiManager.ts contains no forbidden prefixes', () => {
    ...
    // also validates at runtime via phantomchatIntercept.
  ...
  it('PHANTOMCHAT_BRIDGE_METHODS in apiManager.ts includes all required methods', () => {
    // Mirrors the declaration in src/lib/appManagers/apiManager.ts.
