...
describe('api-manager-stub MTProto rejection', () => {
  let installApiManagerStub: typeof import('@lib/phantomchat/api-manager-stub').installApiManagerStub;
  let uninstallApiManagerStub: typeof import('@lib/phantomchat/api-manager-stub').uninstallApiManagerStub;
  ...
  beforeEach(async() => {
    ...
    (window as any).__phantomchatChatAPI = {
      getHistory: vi.fn().mockReturnValue([])
    };

    // Import the module and install the stub explicitly
    const mod = await import('@lib/phantomchat/api-manager-stub');
  ...
  it('routes messages.getHistory for P2P peer through PhantomChat.chat bridge', async() => {
  ...
  it('routes users.getFullUser for P2P peer through PhantomChat.chat bridge', async() => {
