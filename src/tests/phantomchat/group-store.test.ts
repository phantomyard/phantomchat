...
// @lib/phantomchat/group-store with a fake factory. Under isolate:false
...
beforeAll(async() => {
  ...
  vi.doUnmock('@lib/phantomchat/group-store');
  ...
  const storeMod = await import('@lib/phantomchat/group-store');
  GroupStore = storeMod.GroupStore;

  const typesMod = await import('@lib/phantomchat/group-types');
...
describe('GroupStore', () => {
  ...
  beforeEach(async() => {
    ...
    await new Promise<void>((resolve, reject) => {
      const req = indexedDB.deleteDatabase('phantomchat-groups');
