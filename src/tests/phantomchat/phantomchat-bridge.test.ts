/**
 * Unit tests for PhantomChat.chat Bridge modules
 *
...
import {
  ...
  VIRTUAL_PEER_BASE,
  VIRTUAL_PEER_RANGE
} from '../../lib/phantomchat/phantomchat-bridge';
import {
...
describe('PhantomChat.chat Bridge Determinism', () => {
...
describe('PhantomChat.chat Bridge Round-Trip (IndexedDB)', () => {
  let bridge: PhantomChatBridge;
  ...
  beforeEach(async () => {
    ...
    const vdb = await import('../../lib/phantomchat/virtual-peers-db');
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
...
describe('PhantomChat.chat Bridge Avatar Derivation', () => {
  let bridge: PhantomChatBridge;
...
describe('PhantomChatBridge Singleton', () => {
  it('getInstance() returns the same object reference', () => {
    const instance1 = PhantomChatBridge.getInstance();
    const instance2 = PhantomChatBridge.getInstance();
...
describe('api-manager-stub', () => {
  ...
  it('stub logs intercepted methods when flag is enabled', async () => {
    ...
    vi.spyOn(PhantomChatBridge, 'getInstance').mockReturnValue(mockBridge as unknown as PhantomChatBridge);
    ...
    // Call messages.getHistory with a virtual peer — stub should log [PhantomChatStub]
    ...
    expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('[PhantomChatStub]'));
  ...
  it('uninstallApiManagerStub restores original invokeApi', async () => {
    ...
    installApiManagerStub();
    // Clear the module-level "[PhantomChat.chat] apiManager stub installed" log
    consoleSpy.mockClear();
    uninstallApiManagerStub();
