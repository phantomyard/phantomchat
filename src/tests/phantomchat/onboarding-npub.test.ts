...
vi.mock('@lib/phantomchat/migration', () => ({
...
import {PhantomChatOnboarding} from '@/pages/phantomchat/onboarding';
...
describe('PhantomChatOnboarding (npub-based)', () => {
  let onboarding: PhantomChatOnboarding;
  ...
  beforeEach(() => {
    ...
    onboarding = new PhantomChatOnboarding();
  });
  ...
  describe('Create path', () => {
    it('displays npub (not seed phrase) after clicking Create', async() => {
      ...
      // Should show npub in the phantomchat-npub-value element
      const npubDisplay = onboarding.container.querySelector('.phantomchat-npub-value');
  ...
  describe('Import path', () => {
    it('shows 12 input fields for seed phrase', async() => {
      ...
      const inputs = onboarding.container.querySelectorAll('.phantomchat-seed-input');
  ...
  describe('Migration', () => {
    it('runs silently before onboarding if old identity exists', async() => {
      ...
      const {migrateOwnIdToNpub} = await import('@lib/phantomchat/migration');
