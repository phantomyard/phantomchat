...
const APP_URL = 'http://localhost:8080/phantomchat';
...
async function createIdentityWithName(page: Page, displayName: string): Promise<string> {
  ...
  const npub = await page.evaluate(() => {
    ...
    if(w.__phantomchatChatAPI?.ownNpub) return w.__phantomchatChatAPI.ownNpub;
    if(w.__phantomchatNostrRelay?.getPublicKey?.()) return w.__phantomchatNostrRelay.getPublicKey();
    return null;
  });
...
test.describe('Kind 0 profile fetch E2E', () => {
  test('User B sees User A display name from kind 0 after adding without nickname', async() => {
    ...
    const addedViaConsole = await pageB.evaluate(async(pubkey: string) => {
      const w = window as any;
      const bridge = w.__phantomchatDisplayBridge?.bridge;
      if(!bridge) return false;

      // Use the bridge to add peer by pubkey
      const phantomchatBridge = w.__phantomchatBridge || bridge.bridge;
      if(phantomchatBridge?.addPeerByPubkey) {
        await phantomchatBridge.addPeerByPubkey(pubkey);
        return true;
      }
    ...
    const displayName = await pageB.evaluate((pubkey: string) => {
      ...
      const displayBridge = w.__phantomchatDisplayBridge?.bridge;
    ...
    if(displayName) {
      ...
      const profileFetched = await pageB.evaluate(() => {
        return !!(window as any).__phantomchatLastProfileFetch;
      });
