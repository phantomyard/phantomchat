 ...
 *   - Legacy phantomchat-profile-extras migration
 *   - Cache-first hydration dispatches phantomchat_identity_updated
...
vi.mock('@lib/phantomchat/nostr-profile', async() => {
  return {
...
import {
  ...
  saveCachedProfile,
  clearCachedProfile
} from '@lib/phantomchat/profile-cache';
import {
  hydrateOwnProfileFromCache,
  refreshOwnProfileFromRelays,
  saveOwnProfileLocal
} from '@lib/phantomchat/own-profile-sync';
...
describe('profile-cache', () => {
  ...
  it('migrates legacy phantomchat-profile-extras to the new cache key', () => {
    localStorage.setItem('phantomchat-profile-extras', JSON.stringify({
    ...
    expect(localStorage.getItem('phantomchat-profile-extras')).toBeNull();
    // New key must contain the migrated data
    expect(localStorage.getItem('phantomchat-profile-cache')).not.toBeNull();
  });

  it('clears both new and legacy keys', () => {
    saveCachedProfile({profile: {name: 'x'}, created_at: 1});
    localStorage.setItem('phantomchat-profile-extras', '{}');

    clearCachedProfile();

    expect(localStorage.getItem('phantomchat-profile-cache')).toBeNull();
    expect(localStorage.getItem('phantomchat-profile-extras')).toBeNull();
  });
});
...
describe('own-profile-sync: hydrate', () => {
  ...
  it('dispatches phantomchat_identity_updated from the cache', () => {
    saveCachedProfile({
      profile: {
        name: 'alice',
        display_name: 'Alice',
    ...
    expect(eventName).toBe('phantomchat_identity_updated');
