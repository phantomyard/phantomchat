...
describe('Sidebar menu consistency', () => {
  ...
  it('still has PhantomChat.chat profile menu builder', () => {
    expect(sidebarSrc).toContain('buildPhantomChatProfileMenuContent');
  });
});
...
describe('EditProfile save integration', () => {
  ...
  it('save handler dispatches phantomchat_identity_loaded event', () => {
    expect(editProfileSrc).toContain('phantomchat_identity_loaded');
  });
...
describe('Cross-file import integrity', () => {
  ...
  it('privacyAndSecurity.ts imports phantomchatSecurity and the class exists', () => {
    const privacySrc = readFile('components/sidebarLeft/tabs/privacyAndSecurity.ts');
    if(privacySrc.includes('AppPhantomChatSecurityTab')) {
      const securitySrc = readFile('components/sidebarLeft/tabs/phantomchatSecurity.ts');
      expect(securitySrc).toContain('class AppPhantomChatSecurityTab');
    }
  });
});
