...
import {
  ...
  fetchVapidPublicKey,
  buildNip98Header
} from '@lib/phantomchat/phantomchat-push-client';
import {destroy as destroyStorage, setSubscription} from '@lib/phantomchat/phantomchat-push-storage';
...
describe('phantomchat-push-client', () => {
  beforeEach(async() => {
  ...
  describe('buildNip98Header', () => {
    it('produces a "Nostr <base64>" header that decodes to a kind-27235 event', async() => {
      ...
      const url = `https://push.phantomchat.chat/subscription/${pkHex}`;
  ...
  describe('fetchVapidPublicKey', () => {
    it('returns the key on 200', async() => {
      ...
      const k = await fetchVapidPublicKey({endpointBase: 'https://push.phantomchat.chat', fetchFn});
      expect(k).toBe(VAPID_KEY);
      expect(fetchFn).toHaveBeenCalledWith('https://push.phantomchat.chat/info', {method: 'GET'});
    });
  ...
  describe('subscribePush', () => {
    ...
    it('PUTs the right URL + body + NIP-98 header on 200, persists record', async() => {
      ...
      expect(url).toBe(`https://push.phantomchat.chat/subscription/${pkHex}`);
  ...
  describe('unsubscribePush', () => {
    it('issues DELETE with NIP-98 auth and clears storage', async() => {
      ...
      await setSubscription({
        ...
        endpointBase: 'https://push.phantomchat.chat',
        pubkey: pkHex,
