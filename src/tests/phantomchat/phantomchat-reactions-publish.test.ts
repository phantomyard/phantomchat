...
describe('phantomchat-reactions-publish', () => {
  let publishMod: any;
  let storeMod: any;
  let mockChatAPI: any;
  ...
  afterEach(async () => {
    await storeMod?.phantomchatReactionsStore?.destroy?.();
  });
  ...
  beforeEach(async () => {
    ...
    storeMod = await import('@lib/phantomchat/phantomchat-reactions-store');
    publishMod = await import('@lib/phantomchat/phantomchat-reactions-publish');
    publishMod.setChatAPI(mockChatAPI);
    await storeMod.phantomchatReactionsStore.init();
  });

  it('publish() emits kind-7 with e/p tags + emoji content', async () => {
    await publishMod.phantomchatReactionsPublish.publish({
      targetEventId: 'evtX',
      targetMid: 42,
      targetPeerId: 1e16,
      targetAuthor: 'peerpk',
  ...
  it('publish() persists row with reactionEventId from published event', async () => {
    ...
    const rows = await storeMod.phantomchatReactionsStore.getByTarget('evtX');
  ...
  it('unpublish() emits kind-5 delete referencing the reaction event id', async () => {
    ...
    await publishMod.phantomchatReactionsPublish.unpublish('fakeid-0');
  ...
  it('publish() does not duplicate p-tag when targetAuthor === peer', async () => {
    await new Promise<void>((resolve, reject) => {
      const req = (globalThis as any).indexedDB.deleteDatabase('phantomchat-virtual-peers');
      req.onsuccess = () => resolve();
      req.onerror = () => reject(req.error);
      req.onblocked = () => resolve();
    });
