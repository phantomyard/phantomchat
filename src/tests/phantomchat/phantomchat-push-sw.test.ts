...
vi.mock('@lib/phantomchat/phantomchat-identity-sw', () => ({
...
import {unwrapNip17Message} from '@lib/phantomchat/nostr-crypto';
...
describe('phantomchat-push SW handler', () => {
  ...
  it('drops non-phantomchat payloads', async() => {
    await onPhantomChatPush(buildEvent({app: 'telegram'}));
  ...
  it('preview A renders generic title/body without decrypting', async() => {
    ...
    expect(opts.tag).toBe('phantomchat-evt1');
  });
  ...
  it('aggregates 3 quick messages from same peer into one notification body', async() => {
    ...
    await onPhantomChatPush(buildEvent({app: 'phantomchat-webpush-relay', version: 1, event_id: 'e1', recipient_pubkey: 'r', phantomchat_event: '{}'}));
    await onPhantomChatPush(buildEvent({app: 'phantomchat-webpush-relay', version: 1, event_id: 'e2', recipient_pubkey: 'r', phantomchat_event: '{}'}));
    await onPhantomChatPush(buildEvent({app: 'phantomchat-webpush-relay', version: 1, event_id: 'e3', recipient_pubkey: 'r', phantomchat_event: '{}'}));
    ...
    expect(opts3.tag).toBe('phantomchat-same_peer');
  });

  it('does not aggregate after window expires (state forced > 5min ago)', async() => {
    await setPreviewLevel('A');
    await setAggregationState({'evt99': {ts: Date.now() - 6 * 60 * 1000, count: 5, tag: 'phantomchat-evt99'}});
    await onPhantomChatPush(buildEvent({app: 'phantomchat-webpush-relay', version: 1, event_id: 'evt99', recipient_pubkey: 'r', phantomchat_event: '{}'}));
    const [, opts] = showNotification.mock.calls[0];
    expect(opts.body).toBe('New message');
  });
  ...
  it('click handler closes notification and calls openWindow when no client open', async() => {
    ...
    const close = vi.fn();
    const event = {
      notification: {data: {app: 'phantomchat', peerKey: 'pk', eventId: 'eid'}, close},
      waitUntil: () => { /* noop */ }
    } as any;
    await onPhantomChatNotificationClick(event);
