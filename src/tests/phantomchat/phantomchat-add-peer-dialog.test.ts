/**
 * Tests for PhantomChat.chat Add Peer Dialog
 */

import '../setup';
// Use relative path for the page component
import {validateOwnId, OWN_ID_PATTERN, PhantomChatAddPeerDialog} from '../../pages/phantomchat-add-peer-dialog';
...
// ==================== PhantomChatAddPeerDialog Tests ====================

describe('PhantomChatAddPeerDialog', () => {
  let mockOnConnect: ReturnType<typeof vi.fn>;

  beforeEach(() => {
    mockOnConnect = vi.fn();
    // Ensure clean DOM state for each test
    const existing = (window as any).__phantomchatAddPeerDialog as PhantomChatAddPeerDialog | undefined;
    if(existing && !existing.isDestroyed()) {
      existing.destroy();
    }
    delete (window as any).__phantomchatAddPeerDialog;
  });
  ...
  describe('create factory', () => {
    test('creates a dialog instance without showing it', () => {
      const dialog = PhantomChatAddPeerDialog.create(mockOnConnect);

      expect(dialog).toBeInstanceOf(PhantomChatAddPeerDialog);
      expect(dialog.isDestroyed()).toBe(false);

      // Overlay should not be visible yet (display: none)
      const overlay = document.body.querySelector('.phantomchat-dialog-overlay') as HTMLElement | null;
    ...
    test('appends overlay to document.body', () => {
      const dialog = PhantomChatAddPeerDialog.create(mockOnConnect);
      const overlay = document.body.querySelector('.phantomchat-dialog-overlay');
      expect(overlay).toBeTruthy();
      dialog.destroy();
    });
    ...
    test('creates dialog container inside overlay', () => {
      const dialog = PhantomChatAddPeerDialog.create(mockOnConnect);
      const overlay = document.body.querySelector('.phantomchat-dialog-overlay');
      const container = overlay!.querySelector('.phantomchat-dialog');
      expect(container).toBeTruthy();
      dialog.destroy();
    });

    test('multiple create calls create separate overlays', () => {
      const dialog1 = PhantomChatAddPeerDialog.create(mockOnConnect);
      const dialog2 = PhantomChatAddPeerDialog.create(mockOnConnect);

      const overlays = document.body.querySelectorAll('.phantomchat-dialog-overlay');
  ...
  describe('show/hide lifecycle', () => {
    test('show() makes overlay visible', () => {
      ...
      const overlay = document.body.querySelector('.phantomchat-dialog-overlay') as HTMLElement;
    ...
    test('show() renders dialog content', () => {
      ...
      expect(document.querySelector('#phantomchat-ownid-input')).toBeTruthy();
      expect(document.querySelector('#phantomchat-dialog-cancel')).toBeTruthy();
      expect(document.querySelector('#phantomchat-dialog-connect')).toBeTruthy();
    ...
    test('show() renders dialog heading', () => {
      ...
      const heading = document.querySelector('.phantomchat-dialog-header h2');
    ...
    test('hide() clears the input value', () => {
      ...
      const input = document.querySelector('#phantomchat-ownid-input') as HTMLInputElement;
      input.value = 'TEST';
      dialog.hide();

      // After hide, input should be cleared
      const inputAfter = document.querySelector('#phantomchat-ownid-input') as HTMLInputElement;
    ...
    test('show() is idempotent (no re-render on second call)', () => {
      ...
      const inputs = document.querySelectorAll('#phantomchat-ownid-input');
  ...
  describe('Cancel button', () => {
    test('Cancel button hides the dialog', () => {
      ...
      const cancelBtn = document.querySelector('#phantomchat-dialog-cancel') as HTMLButtonElement;
      cancelBtn.click();
  ...
  describe('Connect button validation', () => {
    test('Connect button with invalid OwnID shows error without calling onConnect', async () => {
      ...
      // Click connect
      const connectBtn = document.querySelector('#phantomchat-dialog-connect') as HTMLButtonElement;
      connectBtn.click();
      ...
      const errorEl = document.querySelector('#phantomchat-ownid-error') as HTMLElement;
