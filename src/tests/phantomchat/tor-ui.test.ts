...
describe('phantomchat_tor_circuit_update event type', () => {
  let rootScope;
  ...
  it('should accept circuit update payload shape', () => {
    const handler = vi.fn();
    rootScope.addEventListener('phantomchat_tor_circuit_update', handler);
    ...
    rootScope.dispatchEvent('phantomchat_tor_circuit_update', payload);
    expect(handler).toHaveBeenCalledWith(payload);

    rootScope.removeEventListener('phantomchat_tor_circuit_update', handler);
  });
});
...
describe('WebtorClient circuit details', () => {
  it('should expose getCircuitDetails() returning node fingerprints', async() => {
    ...
    vi.resetModules();
    const {WebtorClient} = await import('@lib/phantomchat/webtor-fallback');
...
describe('PrivacyTransport circuit event dispatch', () => {
  ...
  it('should dispatch phantomchat_tor_circuit_update on circuit polling', () => {
    const handler = vi.fn();
    rootScope.addEventListener('phantomchat_tor_circuit_update', handler);

    rootScope.dispatchEvent('phantomchat_tor_circuit_update', {
      guard: 'AAAA',
      middle: 'BBBB',
      exit: 'CCCC',
      latency: 300,
...
describe('PrivacyTransport.readMode — migration shim', () => {
  beforeEach(() => {
    localStorage.removeItem('phantomchat-tor-mode');
  ...
  it('returns "when-available" when nothing is stored (fresh install default)', async() => {
    const {PrivacyTransport} = await import('@lib/phantomchat/privacy-transport');
    expect(PrivacyTransport.readMode()).toBe('when-available');
  });

  it('returns the stored new-key value verbatim', async() => {
    const {PrivacyTransport} = await import('@lib/phantomchat/privacy-transport');
    localStorage.setItem('phantomchat-tor-mode', 'only');
    ...
    localStorage.setItem('phantomchat-tor-mode', 'when-available');
    expect(PrivacyTransport.readMode()).toBe('when-available');
  });

  it('maps legacy "false" → "off"', async() => {
    const {PrivacyTransport} = await import('@lib/phantomchat/privacy-transport');
    localStorage.setItem('phantomchat-tor-enabled', 'false');
    expect(PrivacyTransport.readMode()).toBe('off');
  });

  it('maps legacy "true" → "when-available" (UX-preserving migration)', async() => {
    const {PrivacyTransport} = await import('@lib/phantomchat/privacy-transport');
    localStorage.setItem('phantomchat-tor-enabled', 'true');
    expect(PrivacyTransport.readMode()).toBe('when-available');
  });

  it('treats garbage in new key as unset and falls through to legacy/default', async() => {
    const {PrivacyTransport} = await import('@lib/phantomchat/privacy-transport');
    localStorage.setItem('phantomchat-tor-mode', 'yes');
    expect(PrivacyTransport.readMode()).toBe('when-available');
    localStorage.setItem('phantomchat-tor-enabled', 'false');
    expect(PrivacyTransport.readMode()).toBe('off');
  });
  ...
  it('setMode writes the new key and clears the legacy key', async() => {
    localStorage.setItem('phantomchat-tor-enabled', 'true');
    const {PrivacyTransport: PT} = await import('@lib/phantomchat/privacy-transport');
    PT.setModeStatic('only');
    expect(localStorage.getItem('phantomchat-tor-mode')).toBe('only');
    expect(localStorage.getItem('phantomchat-tor-enabled')).toBeNull();
  });
});

describe('NostrRelay dual latency tracking', () => {
  it('should store directLatencyMs and torLatencyMs separately', async() => {
    const {NostrRelay} = await import('@lib/phantomchat/nostr-relay');
...
describe('PrivacyTransport (with mocked rootScope)', () => {
  ...
  beforeAll(async() => {
    ...
    const mod = await import('@lib/phantomchat/privacy-transport');
  ...
  it('fires phantomchat_tor_circuit_update on rootScope when onCircuitChange callback is invoked', () => {
  ...
  it('does not dispatch phantomchat_tor_circuit_update when getCircuitDetails returns null', () => {
