/**
 * PhantomChat.chat Onboarding Integration E2E Tests
 *
 * Tests the tweb auth-page interception flow:
 * 1. User visits /?phantomchat=1 without identity → sees PhantomChatOnboarding UI
...
test.describe('PhantomChat.chat Onboarding Integration (tweb)', () => {
  test('sees-onboarding-ui-then-generates-identity-and-sees-ownid', async() => {
    ...
    try {
      ...
      page.on('console', (msg) => {
        const text = msg.text();
        if(text.includes('[PhantomChat.chat')) logs.push(text);
      });

      // Navigate to tweb root with phantomchat=1 flag
      await page.goto('http://localhost:8080/?phantomchat=1', {
        waitUntil: 'networkidle',
        timeout: 20_000
      });

      // Wait for PhantomChatOnboarding to be mounted (container is moved to .scrollable)
      await page.waitForSelector('.phantomchat-onboarding-wrapper.phantomchat-contained', {timeout: 15_000});
      ...
      expect(logs.some((l) => l.includes('[PhantomChatOnboarding]'))).toBe(true);
  ...
  test('reload-with-identity-lands-in-chat', async() => {
    ...
    try {
      ...
      // Navigate with phantomchat=1 — should skip onboarding and mount chat
      await page.goto('http://localhost:8080/?phantomchat=1', {
        waitUntil: 'networkidle',
        timeout: 20_000
      });
      ...
      const flagEnabled = await page.evaluate(() => (window as any).__phantomchatEnabled);
      expect(flagEnabled).toBe(true);

      console.log('[Test] Reload with identity: PhantomChat.chat flag enabled =', flagEnabled);
      await page.close();
      await ctx.close();
    } finally {
      await browser.close();
  ...
  test('window-phantomchatenabled-flag-is-readable', async() => {
    const browser = await chromium.launch({headless: true});
    try {
      const ctx = await browser.newContext();
      const page = await ctx.newPage();
      ...
      // Check window.__phantomchatEnabled is readable
      const flag = await page.evaluate(() => (window as any).__phantomchatEnabled);
