 ...
 * `phantomchatReactionsPublish.publish()`, which in turn publishes a kind-7
...
describe('VMT messages.sendReaction handler', () => {
  ...
  beforeEach(async() => {
    ...
    vi.doMock('@lib/phantomchat/phantomchat-reactions-publish', () => ({
      phantomchatReactionsPublish: {publish: publishSpy, unpublish: vi.fn()},
      setChatAPI: vi.fn()
    }));
    vmtMod = await import('@lib/phantomchat/virtual-mtproto-server');
  });

  it('handles messages.sendReaction -> calls phantomchatReactionsPublish.publish', async() => {
    ...
    const server = new vmtMod.PhantomChatMTProtoServer({
