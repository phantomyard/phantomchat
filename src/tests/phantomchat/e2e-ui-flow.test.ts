/**
 * PhantomChat.chat E2E UI Flow Tests
 *
...
async function injectIdentity(ctx: BrowserContext): Promise<void> {
  ...
  await ctx.addInitScript(
    `(function() {
      ...
      var openReq = indexedDB.open('PhantomChat.chat', 1);
...
function countContainers(page: any): { onboarding: number; chat: number } {
  return page.evaluate(() => {
    return {
      onboarding: document.querySelectorAll('.phantomchat-onboarding-wrapper').length,
      chat: document.querySelectorAll('.pg-chat-page').length
    };
  });
}

test.describe('PhantomChat.chat UI Flow — No Overlapping Containers', () => {
  test('app-router-renders-onboarding-when-no-identity', async() => {
    ...
    try {
      ...
      expect(title).toContain('PhantomChat.chat');
  ...
  test('peer-url-loads-without-crash', async() => {
    ...
    try {
      ...
      await page.goto('http://localhost:8080/phantomchat?peer=AAAAA.BBBBB.CCCCC', {
        waitUntil: 'domcontentloaded',
        timeout: 30000
      });
