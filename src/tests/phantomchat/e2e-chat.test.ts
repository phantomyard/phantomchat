/**
 * PhantomChat.chat E2E Chat Tests
 *
 * Tests the full PhantomChat.chat flow:
...
test.describe('PhantomChat.chat E2E', () => {
  test('app-router-renders-onboarding-when-no-identity', async() => {
    ...
    try {
      ...
      const logs: string[] = [];
      page.on('console', (msg) => {
        if(msg.text().includes('[PhantomChatApp]')) logs.push(msg.text());
      });

      await page.goto('http://localhost:8080/phantomchat', {
      ...
      const appState = await page.evaluate(() => (window as any).__phantomchatApp?.state);
      expect(appState).toBe('onboarding');

      // Verify PhantomChatApp lifecycle logs
  ...
  test('app-router-renders-chat-when-identity-exists', async() => {
    ...
    try {
      ...
      // Navigate to PhantomChat.chat — router should detect identity and render chat
      await page.goto('http://localhost:8080/phantomchat', {
        waitUntil: 'domcontentloaded',
        timeout: 15_000
      });
  ...
  test('two-users-exchange-messages', async() => {
    ...
    try {
      ...
      await Promise.all([
        pageA.goto(
          `http://localhost:8080/phantomchat?peer=${encodeURIComponent(ownIdB)}`,
          {waitUntil: 'domcontentloaded', timeout: 15_000}
        ),
        pageB.goto(
          `http://localhost:8080/phantomchat?peer=${encodeURIComponent(ownIdA)}`,
          {waitUntil: 'domcontentloaded', timeout: 15_000}
        )
      ]);
      ...
      const [stateA, stateB] = await Promise.all([
        pageA.evaluate(() => (window as any).__phantomchatApp?.state),
        pageB.evaluate(() => (window as any).__phantomchatApp?.state)
      ]);
      ...
      const [hasApiA, hasApiB] = await Promise.all([
        pageA.evaluate(() => (window as any).__phantomchatChatAPI !== undefined),
        pageB.evaluate(() => (window as any).__phantomchatChatAPI !== undefined)
      ]);
