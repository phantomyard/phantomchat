...
export default class PopupDeleteMessages {
  ...
  private async construct() {
    ...
    const {peerTitleElement, isBot, messages} = await namedPromises({
      ...
      isBot: managers.appPeersManager.isBot(peerId).catch(() => false),
      messages: Promise.all(mids.map((mid) => managers.appMessagesManager.getMessageByPeer(peerId, mid).catch(() => undefined as Message.message)))
    });

    const isMegagroup = await managers.appPeersManager.isMegagroup(peerId).catch(() => false);
    ...
    const callback = (
      ...
      // P2P delete: handle locally since Worker doesn't have the messages
      ...
      if(peerIdNum >= 1e15 || peerIdNum <= -2e15) {
        ...
        import('@lib/nostra/virtual-mtproto-server').then(async() => {
          ...
          if(proxy) {
            const storageKey = `${Math.abs(peerIdNum)}_history`;
            const cache = proxy.mirrors.messages[storageKey];
            if(cache) mids.forEach((mid: number) => delete cache[mid]);
          }
          // Dispatch history_delete to remove bubbles from DOM
          rootScope.dispatchEvent('history_delete', {peerId, msgs: new Set(mids)});
        });
        return;
      }
    ...
    if(peerId === rootScope.myId || type === ChatType.Scheduled || isBot) {
      ...
      canRevoke = canRevoke.filter((mid, idx) => {
        ...
        if(!message) return true; // P2P: message not in Worker, allow revoke
