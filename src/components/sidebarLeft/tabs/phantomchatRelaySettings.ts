...
import {NostrRelayPool, RelayConfig, DEFAULT_RELAYS} from '@lib/phantomchat/nostr-relay-pool';

const LS_ONLY_MY_RELAYS = 'phantomchat-only-my-relays';

export default class AppPhantomChatRelaySettingsTab extends SliderSuperTab {
  ...
  public init(relayPool?: NostrRelayPool) {
    this.container.classList.add('phantomchat-relay-settings');
    this.setTitle('Nostr Relays' as any);

    this.relayPool = relayPool ?? (window as any).__phantomchatPool ?? null;

    // ─── Preferences ────────────────────────────────────────
    const prefSection = new SettingSection({
      name: 'Preferences' as any,
      caption: 'Ignore the default relays and use only the ones you added' as any
    });
    ...
    rootScope.addEventListener('phantomchat_relay_state', stateHandler);
    this.stateCleanup = () => rootScope.removeEventListener('phantomchat_relay_state', stateHandler);

    const listHandler = () => this.renderCards();
    rootScope.addEventListener('phantomchat_relay_list_changed', listHandler);
    this.listCleanup = () => rootScope.removeEventListener('phantomchat_relay_list_changed', listHandler);
    ...
    const resetSection = new SettingSection({
      caption: 'Restore the list to PhantomChat default relays' as any
    });
  ...
  private updateCaption(connected: number, total: number): void {
    if(this.captionEl) {
      this.captionEl.textContent = `${connected}/${total} connected`;
    }
  }
}
