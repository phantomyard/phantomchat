/*
 * PhantomChat.chat -- Key Protection Settings UI
 *
 * Settings > Key Protection tab: choose PIN/passphrase protection
 * and forgot-PIN recovery via seed phrase import.
 * The seed phrase viewer lives in a separate tab (phantomchatSeedPhrase.ts).
 */
...
import type {EncryptedIdentityRecord} from '@lib/phantomchat/key-storage';

export default class AppPhantomChatSecurityTab extends SliderSuperTab {
  public init() {
    this.container.classList.add('phantomchat-security-settings');
  ...
  private async handleProtectionChange(newType: 'none' | 'pin' | 'passphrase', currentType: string): Promise<void> {
    ...
    try {
      ...
      rootScope.dispatchEvent('phantomchat_identity_updated', {protectionType: newType} as any);
  ...
  private async handleForgotPin(): Promise<void> {
    ...
    try {
      ...
      await saveEncryptedIdentity(updatedRecord);
      rootScope.dispatchEvent('phantomchat_identity_updated', {protectionType: 'none'} as any);
      rootScope.dispatchEvent('phantomchat_identity_unlocked', {npub: record.npub});
