...
import {getAvatarForQR} from '@lib/phantomchat/avatar-for-qr';
...
export default function KeyExchange(props: KeyExchangeProps) {
  const {npub, displayName, nip05, picture} = usePhantomChatIdentity();
  ...
  const handleShare = async() => {
    ...
    try {
      if(typeof navigator.share === 'function') {
        ...
        if(blob) {
          const file = new File([blob as BlobPart], 'phantomchat-qr.png', {type: 'image/png'});
      ...
      instance.download({name: 'phantomchat-qr', extension: 'png'});
    } catch(err) {
