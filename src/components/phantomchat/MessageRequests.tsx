...
import {getMessageRequestStore, MessageRequest} from '@lib/phantomchat/message-requests';
...
export function MessageRequestsList(props: {
  ...
  createEffect(() => {
    ...
    rootScope.addEventListener('phantomchat_message_request', handler);
    return () => {
      rootScope.removeEventListener('phantomchat_message_request', handler);
    };
  });
  ...
  const handleAccept = async(pubkey: string) => {
    try {
      ...
      // Trigger synthetic dialog creation via PhantomChatBridge
      try {
        ...
        rootScope.dispatchEvent('phantomchat_contact_accepted', {pubkey, peerId});
      } catch(bridgeErr) {
 ...
 * Reactively updates on phantomchat_message_request events.
 */
export function useMessageRequestCount(): () => number {
  ...
  // Listen for updates
  rootScope.addEventListener('phantomchat_message_request', () => {
    store.getPendingCount().then(setCount).catch(() => {});
  });
