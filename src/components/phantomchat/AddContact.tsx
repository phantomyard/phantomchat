...
export default function AddContact(props: {
  ...
  return (
    <div class={classNames('phantomchat-add-contact', props.class)}>
      <div class="phantomchat-add-contact-header">
        <button
          class="phantomchat-add-contact-close"
          onClick={() => {
            if(view() !== 'menu') {
              setView('menu');
              setError(null);
        ...
        <h3 class="phantomchat-add-contact-title">Add Contact</h3>
      </div>

      <Show when={error()}>
        <div class="phantomchat-add-contact-error">
          {error()}
        </div>
      </Show>

      <Show when={view() === 'menu'}>
        <div class="phantomchat-add-contact-menu">
          <button
            class="phantomchat-add-contact-option"
            onClick={() => {
              setError(null);
              setView('scan');
            }}
      ...
      <Show when={view() === 'paste'}>
        <div class="phantomchat-add-contact-paste">
          <input
            type="text"
            class="phantomchat-add-contact-input"
          ...
          />
          <button
            class="phantomchat-add-contact-submit"
