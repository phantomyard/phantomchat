...
// the button on an empty input. Re-enabled now that the send/receive path works
// end-to-end (bot-side STT landed); flip to false to disable again if needed.
...
function getPeerExceptions() {
  return _peerExceptions ??= new Set<ChatType>([ChatType.Scheduled, ChatType.Stories, ChatType.Saved]);
}
const PEER_EXCEPTIONS = {has: (v: ChatType) => getPeerExceptions().has(v)};
...
export default class ChatInput {
  ...
  public onMessageSent(clearInput = true, clearReply?: boolean) {
    ...
    emojiEntities.forEach((entity) => {
      const emoji: AppEmoji = entity._ === 'messageEntityEmoji' ? {emoji: entity.unicode ? emojiFromCodePoints(entity.unicode.split('-').map((hex) => parseInt(hex, 16))) : ''} : {docId: entity.document_id, emoji: ''};
