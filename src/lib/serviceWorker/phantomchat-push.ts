/**
 * phantomchat-push.ts (Service Worker)
 *
 * Handles incoming Web Push events shaped for PhantomChat. Discriminator: payload
 * has `app === 'phantomchat-webpush-relay'`. Per preview level:
...
import {loadIdentitySW} from '@lib/phantomchat/phantomchat-identity-sw';
import {hexToBytes} from '@noble/hashes/utils.js';

interface PhantomChatPushPayload {
  app: 'phantomchat-webpush-relay';
  version: 1;
  event_id: string;
  recipient_pubkey: string;
  phantomchat_event: string; // serialized full kind 1059 event
}

const DEFAULT_TITLE = 'PhantomChat';
const DEFAULT_BODY = 'New message';

export async function onPhantomChatPush(event: ExtendableEvent & {data: PushMessageData}): Promise<void> {
  let payload: PhantomChatPushPayload;
  try {
    payload = event.data.json() as PhantomChatPushPayload;
  } catch{
    return;
  }
  if(payload.app !== 'phantomchat-webpush-relay') return;
  ...
  if(previewLevel !== 'A') {
    try {
      ...
      console.warn('[PhantomChatPushSW] decrypt failed:', e?.message);
    }
  }
...
async function tryDecrypt(payload: PhantomChatPushPayload): Promise<DecryptedRumor | null> {
  const identity = await loadIdentitySW();
  if(!identity) return null;
  if(!payload.phantomchat_event) return null;
  const evt = JSON.parse(payload.phantomchat_event);
  const {unwrapNip17Message} = await import('@lib/phantomchat/nostr-crypto');
...
function openVirtualPeersDB(): Promise<IDBDatabase> {
  return new Promise((resolve, reject) => {
    const req = indexedDB.open('phantomchat-virtual-peers');
...
/** Notification click handler — opens or focuses PhantomChat to the right peer. */
export async function onPhantomChatNotificationClick(event: NotificationEvent): Promise<void> {
  const data = event.notification.data;
  if(!data || data.app !== 'phantomchat') return;
  ...
  for(const client of all) {
    try {
      ...
      (client as WindowClient).postMessage({type: 'phantomchat-push-open', url});
      return;
    } catch{ /* ignore */ }
  }
