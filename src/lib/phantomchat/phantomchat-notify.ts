/**
 * phantomchat-notify.ts
 *
 * Main-thread bridge from PhantomChat incoming P2P/group messages to
...
import type {PhantomChatFileMetadata} from '@lib/phantomchat/phantomchat-media-shape';
...
/**
 * Fire a desktop/system notification for an incoming PhantomChat message.
 ...
 * PhantomChat UI; the global Settings → Notifications → Desktop toggle is
...
export async function notifyIncoming(input: NotifyInput, ownPubkey: string): Promise<void> {
  try {
    ...
    const onclick = () => {
      ...
      } catch(e) { logSwallow('PhantomChatNotify.onclick', e); }
    };
    ...
    logSwallow('PhantomChatNotify.notifyIncoming', e);
  }
}
