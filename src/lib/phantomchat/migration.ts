 ...
 *
 * Reads the old PhantomChat.chat/identity store, derives a NIP-06 keypair from
...
import {loadAllQueuedMessages, saveQueuedMessage} from '@lib/phantomchat/offline-queue';
import {nip44Encrypt, getConversationKey} from '@lib/phantomchat/nostr-crypto';
...
// We access the old PhantomChat.chat/identity store directly (not via identity.ts)
// to ensure we close the v1 connection before key-storage opens at v2.

const OLD_DB_NAME = 'PhantomChat.chat';
...
/**
 * Read old identity from PhantomChat.chat/identity store and close the connection.
...
/**
 * Delete old identity record from PhantomChat.chat/identity store and close.
 */
