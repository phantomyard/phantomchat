/*
 * PhantomChat.chat — Own profile sync (cache-first with background refresh)
 *
 ...
 * reads from the phantomchatIdentity store. Dispatches
 * `phantomchat_identity_updated` with the cached fields.
 */
export function hydrateOwnProfileFromCache(): CachedProfile | null {
  const cached = loadCachedProfile();
  if(!cached) return null;

  rootScope.dispatchEventSingle('phantomchat_identity_updated', cachedToEvent(cached.profile));
...
export async function refreshOwnProfileFromRelays(
  ...
  try {
    ...
    rootScope.dispatchEventSingle('phantomchat_identity_updated', cachedToEvent(fresh.profile));
...
export function saveOwnProfileLocal(profile: NostrProfile, created_at: number): void {
  ...
  saveCachedProfile(fresh);
  rootScope.dispatchEventSingle('phantomchat_identity_updated', cachedToEvent(merged));
}
