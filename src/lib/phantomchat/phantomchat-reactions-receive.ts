 ...
 * (PhantomChatSync on new-message ingest) may invoke flushPending(eventId).
 *
 * Author integrity:
...
import {phantomchatReactionsStore, type ReactionRow} from './phantomchat-reactions-store';
...
class PhantomChatReactionsReceive {
  private ownPubkey = '';
  ...
  private async persist(event: NostrEventLite, targetEventId: string, targetMid: number, targetPeerId: number): Promise<void> {
    ...
    await phantomchatReactionsStore.add(row);
    rootScope.dispatchEventSingle('phantomchat_reactions_changed', {
      peerId: targetPeerId,
      mid: targetMid
    });
...
export const phantomchatReactionsReceive = new PhantomChatReactionsReceive();

if(typeof window !== 'undefined') {
  (window as any).__phantomchatReactionsReceive = phantomchatReactionsReceive;
}
