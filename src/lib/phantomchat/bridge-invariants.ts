/*
 * PhantomChat.chat — Virtual MTProto Bridge Invariants
 *
...
const log = logger('PhantomChat/bridge-invariants');

// ─── Constants ────────────────────────────────────────────────────────

/** Lower bound of the synthetic P2P peer-id range. Duplicated from
 *  `phantomchat-bridge.ts` (VIRTUAL_PEER_BASE = BigInt(10 ** 15)) as a plain
 *  number so this module stays Worker-safe (phantomchat-bridge drags in
 *  main-thread-only imports). Keep the two in sync. */
export const P2P_PEER_ID_MIN = 1e15;

/** Action prefixes that MUST NOT appear in PHANTOMCHAT_ACTION_PREFIXES. Query
 *  methods need shaped responses, not a bare `true` (Rule 2). */
export const FORBIDDEN_ACTION_PREFIXES = ['.get', '.check'] as const;

/** Bridge methods that MUST be present in PHANTOMCHAT_BRIDGE_METHODS. Without
...
/** Rule 2: PHANTOMCHAT_ACTION_PREFIXES must not contain `.get` or `.check`.
...
export function validateActionPrefixes(prefixes: ReadonlyArray<string>): InvariantResult {
  for(const forbidden of FORBIDDEN_ACTION_PREFIXES) {
    if(prefixes.includes(forbidden)) {
      return {ok: false, reason: 'PHANTOMCHAT_ACTION_PREFIXES contains forbidden prefix: ' + forbidden};
    }
  }
  return {ok: true};
}

/** Rule 15: PHANTOMCHAT_BRIDGE_METHODS must include the edit/send/history
...
export function validateBridgeMethods(methods: ReadonlySet<string> | ReadonlyArray<string>): InvariantResult {
  ...
  for(const required of REQUIRED_BRIDGE_METHODS) {
    if(!asSet.has(required)) {
      return {ok: false, reason: 'PHANTOMCHAT_BRIDGE_METHODS missing required method: ' + required};
    }
  }
  return {ok: true};
}
 ...
 *  carry `phantomchatMid` (number) + `phantomchatEventId` (string). The Worker's
...
export function validateSendMessageResponse(response: unknown): InvariantResult {
  ...
  if(typeof r.phantomchatMid !== 'number') {
    return {ok: false, reason: 'response.phantomchatMid missing or wrong type'};
  }
  if(typeof r.phantomchatEventId !== 'string' || r.phantomchatEventId.length === 0) {
    return {ok: false, reason: 'response.phantomchatEventId missing or empty'};
  }
  return {ok: true};
}
 ...
 *  `['phantomchat-edit', '<originalAppMessageId>']`. The edit must drop if
...
/** Rule 4: `window.__phantomchatOwnPubkey` must be set once onboarding
...
export function validateOwnPubkeySet(): InvariantResult {
  ...
  const pubkey = (window as unknown as {__phantomchatOwnPubkey?: string}).__phantomchatOwnPubkey;
  if(typeof pubkey !== 'string' || pubkey.length === 0) {
    return {ok: false, reason: 'window.__phantomchatOwnPubkey is not set'};
  }
  return {ok: true};
}
