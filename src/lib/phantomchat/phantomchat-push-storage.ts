/**
 * phantomchat-push-storage.ts
 *
 * IDB wrapper for the `phantomchat-push` database. Used by the main-thread push
 ...
 *
 * Schema (DB: phantomchat-push, version 1):
 *   objectStore 'kv' (keyPath: 'k') — small key/value records
 *   keys:
 *     'subscription'  → PushSubscriptionRecord
 *     'preview_level' → 'A' | 'B' | 'C'  (default 'A')
 *     'endpoint'      → string (override; default 'https://push.phantomchat.chat')
 *     'aggregation'   → Record<peerId, AggregationEntry>
 */

const DB_NAME = 'phantomchat-push';
const DB_VERSION = 1;
const STORE_NAME = 'kv';

export const DEFAULT_ENDPOINT = 'https://push.phantomchat.chat';
 ...
 * phantomchat-cleanup.ts can deleteDatabase without "blocked" race.
 */
