...
export async function deriveKeys(seed: string): Promise<KeyPair> {
  ...
  const signingBits = await crypto.subtle.deriveBits(
    {
      name: 'PBKDF2',
      salt: encoder.encode('PhantomChat.chat-Signing-v1'),
      iterations: 100000,
      hash: 'SHA-256'
    },
    signingKeyMaterial,
  ...
  const encryptionBits = await crypto.subtle.deriveBits(
    {
      name: 'PBKDF2',
      salt: encoder.encode('PhantomChat.chat-Encryption-v1'),
      iterations: 100000,
      hash: 'SHA-256'
    },
 ...
 */
const DB_NAME = 'PhantomChat.chat';
...
export async function loadIdentity(): Promise<StoredIdentity | null> {
  try {
    ...
    try {
      const stored = localStorage.getItem('phantomchat_identity');
...
export async function saveIdentity(identity: Omit<StoredIdentity, 'id' | 'createdAt'>): Promise<void> {
  ...
  try {
    ...
    localStorage.setItem('phantomchat_identity', JSON.stringify(full));
  }
}
...
export async function clearIdentity(): Promise<void> {
  ...
  localStorage.removeItem('phantomchat_identity');
}
