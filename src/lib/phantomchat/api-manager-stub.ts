 ...
 * api-manager-stub
 *
 * Monkey-patches apiManager.invokeApi() to route P2P traffic to PhantomChat.chat
...
function getChatAPI(): ChatAPI | undefined {
  return (window as any).__phantomchatChatAPI as ChatAPI | undefined;
}
...
export function installApiManagerStub(): boolean {
  ...
  if(!apiManager) {
    console.warn('[PhantomChat.chat] apiManager stub: apiManager not found on ctx — stub not installed');
    return false;
  }
  ...
  (apiManager as any).invokeApi = async function<T extends InvokeApiMethod>(
    ...
    if(method === 'messages.getHistory') {
      ...
      if(peerId === null) {
        console.warn(`[PhantomChat.chat] MTProto disabled: ${method} rejected (null peerId)`);
      ...
      if(pubkey === null) {
        console.warn(`[PhantomChat.chat] MTProto disabled: ${method} rejected (unknown peer ${peerId})`);
        return Promise.reject({type: 'MTPROTO_DISABLED', code: 503, description: `Method ${method} is not available - MTProto connections disabled`});
      }

      console.log(`[PhantomChatStub] routing messages.getHistory for peerId=${peerId} (${pubkey.slice(0, 8)}...)`);

      const chatAPI = getChatAPI();
      if(!chatAPI) {
        console.warn(`[PhantomChat.chat] MTProto disabled: ${method} rejected (ChatAPI unavailable)`);
        return Promise.reject({type: 'MTPROTO_DISABLED', code: 503, description: `Method ${method} is not available - MTProto connections disabled`});
      }
    ...
    // --- users.getFullUser: route through PhantomChatBridge ---
    if(method === 'users.getFullUser') {
      const userId = extractUserIdFromGetFullUser(req);
      if(userId === null) {
        console.warn(`[PhantomChat.chat] MTProto disabled: ${method} rejected (null userId)`);
        return Promise.reject({type: 'MTPROTO_DISABLED', code: 503, description: `Method ${method} is not available - MTProto connections disabled`});
      }
      ...
      if(pubkey === null) {
        console.warn(`[PhantomChat.chat] MTProto disabled: ${method} rejected (unknown user ${userId})`);
        return Promise.reject({type: 'MTPROTO_DISABLED', code: 503, description: `Method ${method} is not available - MTProto connections disabled`});
      }

      console.log(`[PhantomChatStub] routing users.getFullUser for userId=${userId} (${pubkey.slice(0, 8)}...)`);
    ...
    if(method === 'messages.getDialogs') {
      console.log(`[PhantomChatStub] returning empty dialogs for ${method}`);
      return {
        _: 'messages.dialogs',
        count: 0,
        dialogs: [],
    ...
    if(method === 'contacts.getContacts') {
      console.log(`[PhantomChatStub] returning empty contacts for ${method}`);
      return {
        _: 'contacts.contacts',
        contacts: [],
        saved_count: 0,
  ...
  stub.installed = true;
  console.log('[PhantomChat.chat] apiManager stub installed');
  return true;
}
