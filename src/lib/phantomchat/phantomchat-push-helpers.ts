/**
 * phantomchat-push-helpers.ts
 *
 * Shared helpers for the push notification subsystem — usable from both the
 * Settings UI (PhantomChatBackgroundNotifications) and the onboarding integration.
 */

import {getEndpointBase} from '@lib/phantomchat/phantomchat-push-storage';
import {fetchVapidPublicKey} from '@lib/phantomchat/phantomchat-push-client';
...
export async function resolveVapidKey(): Promise<string | null> {
  try {
    ...
    const cacheKey = `phantomchat-push-vapid-${base}`;
