...
const LS_MODE_KEY = 'phantomchat-tor-mode';
const LS_LEGACY_KEY = 'phantomchat-tor-enabled';
...
export class PrivacyTransport {
  ...
  constructor(relayPool: NostrRelayPool, offlineQueue: OfflineQueue, webtorClient?: WebtorClient) {
    ...
    if(this.webtorClient) {
      ...
      (this.webtorClient as any)._events = {
        ...
        onCircuitChange: () => {
          ...
          if(details) {
            rootScope.dispatchEvent('phantomchat_tor_circuit_update', details);
          }
        }
      };
    }
    ...
    if(typeof window !== 'undefined') {
      (window as any).__phantomchatPrivacyTransport = this;
    }
  }
   ...
   * existing call sites in phantomchatStatus, tor-ui-state, and phantomchat-bridge
  ...
  async send(recipientPubkey: string, plaintext: string): Promise<PublishResult | null> {
    ...
    if(!canPublish) {
      ...
      rootScope.dispatchEvent('phantomchat_message_queued', {messageId, status: 'queued'});
      return null;
    }
    const result = await this.relayPool.publish(recipientPubkey, plaintext);
    if(result.successes.length > 0) {
      rootScope.dispatchEvent('phantomchat_message_queued', {
  ...
  async setMode(mode: TorMode): Promise<void> {
    ...
    rootScope.dispatchEvent('phantomchat_tor_mode_changed' as any, mode);
    this.stopRetryLoop();
  ...
  private setState(state: PrivacyTransportState, error?: string): void {
    ...
    rootScope.dispatchEvent('phantomchat_tor_state', {
  ...
  private setRuntimeState(next: RuntimeState, error?: string): void {
    ...
    rootScope.dispatchEvent('phantomchat_tor_state', {state: next, error});
  }
