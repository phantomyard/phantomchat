/**
 * WebRTC peer connection manager for PhantomChat.chat
 * Extends tweb's createPeerConnection pattern with PhantomChat.chat-specific ICE configuration
 */
...
/**
 * PhantomChat.chat-specific ICE configuration with public STUN servers
 */
export const PhantomChatIceConfig: RTCConfiguration = {
  iceServers: [
...
/**
 * Create a PhantomChat.chat-specific RTCPeerConnection with PhantomChat.chat ICE config
 * and event handlers for ICE, connection state, and negotiation
 */
export function createPhantomChatPeerConnection(
  config?: RTCConfiguration,
  log?: Logger
): {connection: RTCPeerConnection; log: Logger} {
  // Merge PhantomChat.chat defaults with optional user config
  const mergedConfig: RTCConfiguration = {
    ...PhantomChatIceConfig,
    ...config,
    iceServers: config?.iceServers?.length ?
      config.iceServers :
      PhantomChatIceConfig.iceServers
  };

  if(!log) {
    log = logger('PhantomChatPeerConnection');
  }
  ...
  // Attach PhantomChat.chat-specific handlers for observability
  ...
  if(typeof window !== 'undefined' && localStorage.getItem('pg:transport:debug') === '1') {
    (window as any).__phantomchatPeerConnection = connection;
  }
...
export class PeerChannel {
  ...
  constructor(channel: RTCDataChannel, log?: Logger) {
    ...
    if(typeof window !== 'undefined' && localStorage.getItem('pg:transport:debug') === '1') {
      (window as any).__phantomchatPeerChannel = this;
    }
  }
