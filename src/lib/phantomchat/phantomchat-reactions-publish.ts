...
class PhantomChatReactionsPublish {
  async publish(args: PublishArgs): Promise<string> {
    if(!chatAPI) throw new Error('[phantomchat-reactions-publish] ChatAPI not wired — call setChatAPI first');
    // Build p-tags:
    ...
    if(!reactionEventId) throw new Error('[phantomchat-reactions-publish] published event has no id');
    ...
    rootScope.dispatchEventSingle('phantomchat_reactions_changed', {
  ...
  async unpublish(reactionEventId: string): Promise<void> {
    if(!chatAPI) throw new Error('[phantomchat-reactions-publish] ChatAPI not wired');
    ...
    await phantomchatReactionsStore.removeByReactionEventId(reactionEventId);
    rootScope.dispatchEventSingle('phantomchat_reactions_changed', {
...
export const phantomchatReactionsPublish = new PhantomChatReactionsPublish();

if(typeof window !== 'undefined') {
  (window as any).__phantomchatReactionsPublish = phantomchatReactionsPublish;
}
