 ...
 * `phantomchatReactionsStore` (IDB-backed). Callers use `addReaction()` and
 * `getReactions()` as before; internally:
 *  - addReaction triggers phantomchatReactionsPublish.publish (relay + store)
...
class PhantomChatReactionsLocal {
   ...
   * if called from a `phantomchat_reactions_changed` listener registered before
   ...
   * bubbles.ts both subscribe to `phantomchat_reactions_changed`. Since the
  ...
  private async refreshCache(peerId: number, mid: number): Promise<void> {
    ...
    const rows = await phantomchatReactionsStore.getAll();
...
export const phantomchatReactionsLocal = new PhantomChatReactionsLocal();

if(typeof window !== 'undefined') {
  (window as any).__phantomchatReactionsLocal = phantomchatReactionsLocal;
}
